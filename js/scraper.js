...
function parseNH(html,sourceUrl){
  ...
  // ── KOP ──
  const headline=(doc.querySelector('h1[class*="article_title"]')||doc.querySelector('h1'))?.textContent?.trim()||
    doc.querySelector('meta[property="og:title"]')?.content||'';

  // ── AUTEUR ── (NH doesn't always show author in article body)
  const author=doc.querySelector('[class*="author_name"]')?.textContent?.trim()||
    doc.querySelector('meta[name="author"]')?.content||'';

  // ── DATUM ──
  const pubDate=doc.querySelector('[class*="article_time"]')?.textContent?.replace(/\s+/g,' ').trim()||'';
  ...
  // ── INTRO ──
  const introEl=doc.querySelector('[class*="article-block_intro"]');
  ...
  // Text blocks
  doc.querySelectorAll('[class*="article-block_text"]').forEach(block=>{
  ...
  // Info boxes (like "Tip ons!")
  doc.querySelectorAll('[class*="article-block_info"]').forEach(box=>{
    const h=box.querySelector('h2')?.textContent?.trim();
    const t=[...box.querySelectorAll('p')].map(p=>p.textContent.trim()).filter(t=>t).join(' ');
    if(h||t) rawParts.push('[KADER] '+(h?h+': ':'')+t);
  });
  ...
  // ── IMAGES ──
  const images=[];
  doc.querySelectorAll('[class*="article-block_image"]').forEach(block=>{
    const img=block.querySelector('img');
    if(!img) return;
    const src=(img.getAttribute('src')||'').split('?')[0];
    const desc=block.querySelector('[class*="article-block_description"]')?.textContent?.trim()||img.alt||'';
    if(src&&!images.some(i=>i.src===src)){
      images.push({src,desc});
      rawParts.push('[AFBEELDING] '+src+(desc?' — '+desc:''));
    }
  });

  // ── EMBEDS — Instagram ──
  const embeds=[];
  doc.querySelectorAll('iframe[src*="instagram.com"]').forEach(iframe=>{
    const m=iframe.src.match(/instagram\.com\/(reel|p)\/([A-Za-z0-9_-]+)/);
    if(m){
      const url=`https://www.instagram.com/${m[1]}/${m[2]}/`;
      embeds.push({type:'instagram',val:url,detail:'Instagram embed'});
      rawParts.push('[EMBED INSTAGRAM] '+url);
    }
  });

  // ── RELATED (Lees ook) ──
  const related=[];
  doc.querySelectorAll('[class*="article-block_articles"] a[class*="article-teaser_link"]').forEach(a=>{
  ...
  const urlArticleId=urlMatch?urlMatch[1]:'';
  const category=sourceUrl.match(/\.nl\/([^/]+)\//)?.[1]||'';
  const cleanBodyText=bodyText.replace(/\[.*?\]/g,'').trim();
  const wordCount=cleanBodyText?cleanBodyText.split(/\s+/).filter(w=>w).length:0;
  const readTimeMin=wordCount>0?Math.max(1,Math.round(wordCount/200)):0;
  const internalLinks=links.filter(l=>l.href.includes(domain)).length;
...
function parseArticle(html,sourceUrl){
  ...
  // ── CATEGORIE uit metadata ──
  const category=
    doc.querySelector('meta[property="article:section"]')?.content||
    doc.querySelector('meta[name="category"]')?.content||
    root.querySelector('.category-labels .name-label')?.textContent?.trim()||
    doc.querySelector('.category-labels .name-label')?.textContent?.trim()||
    '';

  // ── EXTRA METADATA ──

  // Leestijd als getal
  const readTimeMatch=pubDate.match(/(\d+)\s*minuten?\s*leestijd/i);
  const readTimeMin=readTimeMatch?parseInt(readTimeMatch[1]):0;

  // Domein uit URL
  let domain='';
  try{domain=new URL(sourceUrl).hostname.replace('www.','').split('.')[0];}catch(e){}

  // Quote auteurs
  const quoteAuthors=[];
  const bodyQuotes=bodyText.match(/\[QUOTE\][^]*?—\s*([^\n]+)/g)||[];
  bodyQuotes.forEach(q=>{const m=q.match(/—\s*(.+)/);if(m&&!quoteAuthors.includes(m[1].trim()))quoteAuthors.push(m[1].trim());});

  // Copyright per afbeelding
  const copyrights=[];
  root.querySelectorAll('[__component="api.api-image"] .copyright, figure .copyright').forEach(el=>{
    const c=el.textContent.trim();if(c&&!copyrights.includes(c))copyrights.push(c);
  });
  ...
  links.forEach(l=>{
    try{
      ...
      const sourceHost=sourceUrl!=='(geplakt)'?new URL(sourceUrl).hostname:'';
      if(linkHost===sourceHost||l.href.startsWith('/'))internalLinks.push(l);
      else externalLinks.push(l);
    }catch(e){internalLinks.push(l);}
  });

  // Tekentelling
  const cleanBodyText=bodyText.replace(/\[[^\]]*\]/g,'').trim();
  const charKop=headline.length;
  const charIntro=intro.length;
  const charTekst=cleanBodyText.length;

  return {headline,author,pubDate,ogImage,intro,bodyText,links,images,embeds,related,
    urlArticleId,sourceUrl,category,readTimeMin,domain,quoteAuthors,copyrights,
    internalLinks,externalLinks,charKop,charIntro,charTekst,
    rawText:finalRaw.join('\n')};
}
...
function exportBulkCSV(){
  ...
  const headers=['artikel_id','url','domein','categorie','kop','auteur','datum','leestijd_min',
    'intro','tekst','og_image',
    'afbeeldingen','interne_links','externe_links','gerelateerd','tussenkoppen','kaders','quotes',
    'audio','video','instagram','twitter','youtube','overig_embeds',
    'quote_auteurs','copyrights',
    'afbeelding_urls','interne_link_urls','externe_link_urls','gerelateerd_urls',
    'audio_urls','video_urls','instagram_urls','twitter_urls','youtube_urls',
    'wrd_kop','wrd_intro','wrd_tekst','chr_kop','chr_intro','chr_tekst','alineas','ruw'];

  function csvVal(v){const s=String(v).replace(/"/g,'""');return s.includes(',')||s.includes('"')||s.includes('\n')?'"'+s+'"':s;}

  function embedUrls(a,type){return a.embeds.filter(e=>e.type===type&&e.val&&!e.val.includes('gedetecteerd')).map(e=>e.val).join(' | ');}
  ...
  function articleRow(a){
    ...
    return [
      ...
      a.domain||'',
      a.category||'',
      a.headline||'',a.author||'',a.pubDate||'',a.readTimeMin||0,
      a.intro||'',a.bodyText||'',a.ogImage||'',
      a.images.length+(a.ogImage?1:0),
      (a.internalLinks||[]).length,(a.externalLinks||[]).length,
      a.related.length,
      ...
      (a.internalLinks||[]).map(l=>l.href).join(' | '),
      (a.externalLinks||[]).map(l=>l.href).join(' | '),
      ...
      a.charKop||0,a.charIntro||0,a.charTekst||0,
      alineas,a.rawText||''
    ].map(csvVal).join(',');
  }
...
function renderTableHTML(a){
  ...
  if(a.category) html+=srRow('Categorie',a.category);
